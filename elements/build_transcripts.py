...
def build_genes( exons, tss_exons, tes_exons, jns ):
    ...
    for key in sorted(keys):
        chrm, strand = key
        all_chrm_exons = numpy.vstack( (exons[key], tss_exons[key], tes_exons[key]) )
        
        genelets = cluster_exons( all_chrm_exons, jns[key] )
        
        # find tss and tes exons and add the grouped exons to the clustered_exons list
...
def parse_arguments():
    ...
    return args.exons, args.tss, args.tes, args.junctions, \
        args.threads, out_fp, log_fp

def main():
    exon_fps, tss_exon_fps, tes_exon_fps, junction_fps, \
        threads, out_fp, log_fp = parse_arguments()
